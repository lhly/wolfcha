...
export function AnalysisFooter({ onShare, onReturn }: AnalysisFooterProps) {
  return (
    <footer className="pt-2 pb-6 flex flex-col gap-4">
      ...
      <button
        onClick={onReturn}
        className="w-full bg-transparent text-[var(--color-gold)]/60 font-bold py-4 rounded-lg hover:bg-white/5 hover:text-[var(--color-gold)] transition border border-[var(--color-gold)]/20 tracking-wider text-xs"
      >
        RETURN TO LOBBY
      </button>
    </footer>
